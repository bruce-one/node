...
const { Serializer, Deserializer, serialize, deserialize } = require('v8');
...
{
  for (const obj of objects) {
    assert.deepStrictEqual(deserialize(serialize(obj)), obj);
  }
}

{
  const ser = new Serializer({
    getDataCloneError: common.mustCall((message) => {
...
{
  const ser = new Serializer({
    writeHostObject: common.mustCall((object) => {
  ...
  const des = new Deserializer(ser.releaseBuffer(), {
    readHostObject: common.mustCall(() => {
