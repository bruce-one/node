...
const {
  writeInfoBuffer, kStreamDispatchedBytes
...
const kCurrentWriteBytes = Symbol('kCurrentWriteBytes');
const kCurrentWriteCb = Symbol('kCurrentWriteCb');
const kCurrentWriteBuffer = Symbol('kCurrentWriteBuffer');
...
// TODO(addaleax): Merge with lib/net.js
function createWriteReq(handle, data, encoding) {
  switch (encoding) {
    ...
    case 'latin1':
    case 'binary':
      return handle.writeLatin1String(data);
    case 'buffer':
      return handle.writeBuffer(data);
    default:
      const buffer = Buffer.from(data, encoding);
      handle[kOwner][kCurrentWriteBuffer] = buffer;
      return handle.writeBuffer(buffer);
  }
}
...
function afterDoStreamWrite(status) {
  const stream = this[kOwner];
  const session = stream[kSession];

  stream[kUpdateTimer]();

  stream[kState].writeQueueSize -= stream[kCurrentWriteBytes];
  if (session !== undefined)
    session[kState].writeQueueSize -= stream[kCurrentWriteBytes];

  stream[kCurrentWriteBuffer] = null;

  if (typeof stream[kCurrentWriteCb] === 'function')
    stream[kCurrentWriteCb](null);
}
...
class Http2Stream extends Duplex {
  constructor(session, options) {
    ...
    this[kCurrentWriteCb] = null;
    this[kCurrentWriteBuffer] = null;
    this[kCurrentWriteBytes] = 0;
  ...
  [kInit](id, handle) {
    ...
    handle.onaftershutdown = () => {};
    handle.onafterwrite = afterDoStreamWrite;
    this.uncork();
    this.emit('ready');
  }
  ...
  _write(data, encoding, cb) {
    ...
    const handle = this[kHandle];
    const err = createWriteReq(handle, data, encoding);
    if (err)
      throw util._errnoException(err, 'write', handle.error);

    this[kCurrentWriteCb] = cb;
    this[kCurrentWriteBytes] = writeInfoBuffer[kStreamDispatchedBytes];
    if (encoding === 'buffer')
      this[kCurrentWriteBuffer] = data;  // Retain memory until write finishes

    trackWriteState(this, writeInfoBuffer[kStreamDispatchedBytes]);
  }
  ...
  _writev(data, cb) {
    ...
    const err = handle.writev(chunks);
    if (err)
      throw util._errnoException(err, 'write', handle.error);

    this[kCurrentWriteCb] = cb;
    this[kCurrentWriteBytes] = writeInfoBuffer[kStreamDispatchedBytes];
    this[kCurrentWriteBuffer] = chunks;  // Retain chunks until write finishes
