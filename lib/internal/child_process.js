...
const {
  writeInfoBuffer, kStreamWriteAsyncFlag
...
function setupChannel(target, channel) {
  ...
  const kWriteInProgess = Symbol('kWriteInProgess');

  function drainWriteQueue() {
    if (target[kWriteQueue].length === 0)
      return;
    const args = target[kWriteQueue].shift();
    if (target._send(...args))
      process.nextTick(drainWriteQueue);
  }

  target[kWriteQueue] = [];
  target[kWriteInProgess] = false;
  target._send = function(message, handle, options, callback) {
    ...
    if (this[kWriteInProgess]) {
      this[kWriteQueue].push([message, handle, options, callback]);
      return false;
    }
    ...
    channel.onafterwrite = () => {};
    const string = JSON.stringify(message) + '\n';
    const err = channel.writeUtf8String(string, handle);
    const async = writeInfoBuffer[kStreamWriteAsyncFlag];
    this[kWriteInProgess] = async;
