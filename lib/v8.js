...
class Serializer extends serdesBinding.Serializer {
  constructor(options = {}) {
    ...
    if (typeof options.getDataCloneError !== 'function' &&
        typeof options.getDataCloneError !== 'undefined') {
      throw new TypeError('getDataCloneError must be a function or undefined');
    }

    if (typeof options.writeHostObject !== 'function' &&
        typeof options.writeHostObject !== 'undefined') {
      throw new TypeError('writeHostObject must be a function or undefined');
    }

    if (typeof options.writeArrayBufferView !== 'function' &&
        typeof options.writeArrayBufferView !== 'undefined') {
      throw new TypeError('writeArrayBufferView must be a function ' +
                          'or undefined');
    }

    this.getDataCloneError = options.getDataCloneError || Error;
    this.writeHostObject = options.writeHostObject;

    if (options.writeArrayBufferView) {
      this.writeArrayBufferView = options.writeArrayBufferView;
    }

    if (options.treatArrayBufferViewsAsHostObjects !== false) {
      this.setTreatArrayBufferViewsAsHostObjects(true);
    }
  }

  writeArrayBufferView(abView) {
    const i = arrayBufferViewTypeToIndex[objectToString(abView)];
    this.writeUint32(i);
...
class Deserializer extends serdesBinding.Deserializer {
  constructor(buffer, options = {}) {
    super(buffer);

    if (typeof options.readHostObject !== 'function' &&
        typeof options.readHostObject !== 'undefined') {
      throw new TypeError('writeHostObject must be a function or undefined');
    }

    if (Object.prototype.hasOwnProperty.call(options, 'readHostObject')) {
      this.readHostObject = options.readHostObject;
    }
  }

  readRawBytes(length) {
    const offset = this._readRawBytes(length);
    // `this.buffer` can be a Buffer or a plain Uint8Array.
  ...
  readHostObject() {
    ...
    return new ctor(this.buffer.buffer,
                    ...
                    byteLength / (ctor.prototype.BYTES_PER_ELEMENT || 1));
  }
}
...
exports.deserialize = function deserialize(buffer) {
  const der = new Deserializer(buffer);
  der.readHeader();
