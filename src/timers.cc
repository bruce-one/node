...
class TimerWrap : public HandleWrap {
 public:
  TimerWrap(Environment* env,
    ...
    : HandleWrap(env, wrap, reinterpret_cast<uv_handle_t*>(&timer_), PROVIDER_TIMER) {
  ...
  static void Start(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    double delay_ms = args[0].As<Number>()->Value();

    uv_timer_start(&wrap->timer_, Callback, delay_ms, 0);
  }
